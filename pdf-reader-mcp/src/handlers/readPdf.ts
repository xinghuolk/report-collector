  ...
  if (includeMetadata) {
    try {
      ...
      if (metadataObj && typeof metadataObj === 'object') {
        // Handle different metadata formats depending on pdfjs-dist version
        if ('getAll' in metadataObj && typeof metadataObj.getAll === 'function') {
          const metadataData = metadataObj.getAll() as PdfMetadata | undefined;
          if (metadataData !== undefined) {
            output.metadata = metadataData;
          }
        } else {
          // Fallback: treat metadata object directly as metadata
          output.metadata = metadataObj as unknown as PdfMetadata;
        }
      }
