...
This is upstream of task 6. The semantic and canonical layers cannot fix structure that is already missing when `PdfTableSource` returns the raw table blocks.
...
Writing those as the main regression tests right now would not be a healthy red-green cycle for the current codebase. The failures would mostly reflect missing upstream capabilities rather than small compatibility gaps in task 6 itself.

The main risk is accidentally freezing today's weak output as the intended behavior. Task 6 should stay target-oriented, and the missing capabilities should be fixed before the main sample assertions are added.

## Recommended Next Step

Pause the main task-6 target regression work and add a separate capability-recovery step focused on HK annual structure recovery.
