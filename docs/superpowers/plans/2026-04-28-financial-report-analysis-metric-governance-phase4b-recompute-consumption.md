...
- Modify `financial-report-analysis/src/financial_report_analysis/api/routes.py`: add read-only lifecycle recompute audit endpoint.
...
def test_audit_marks_mapped_and_blacklisted_items_recompute_needed() -> None:
    mapped_item = _review_item("review:mapped", metric_id="custom::cn::general::balance-sheet::root::deposit")
    blacklisted_item = _review_item("review:blacklisted", metric_id="custom::cn::general::income-statement::root::bad")

    audit = build_metric_lifecycle_recompute_audit(
        review_items=(mapped_item, blacklisted_item),
        lifecycle_state_loader=lambda review_item_id: {
            "review:mapped": _state("mapped_to_standard", action="map_to_standard"),
            "review:blacklisted": _state("blacklisted", action="blacklist"),
        }[review_item_id],
    )
...
def test_audit_leaves_non_output_states_not_recompute_needed() -> None:
    items = (
        _review_item("review:approved"),
        _review_item("review:deprecated"),
        _review_item("review:provisional"),
    )

    audit = build_metric_lifecycle_recompute_audit(
        review_items=items,
        lifecycle_state_loader=lambda review_item_id: {
            "review:approved": _state("approved_custom", action="approve_custom"),
            "review:deprecated": _state("deprecated", action="deprecate"),
            "review:provisional": _state_without_decision("provisional"),
        }[review_item_id],
    )
...
`_state_without_decision` helpers using existing governance dataclasses.
...
def get_metric_lifecycle_recompute_audit(
    ...
    if dry_run:
        artifact_loader = lambda artifact_id: _load_artifact_for_lifecycle_dry_run(
            repository,
            artifact_id,
        )
    audit = build_metric_lifecycle_recompute_audit(
...
Add response conversion helpers in `routes.py`. Wrap dry-run artifact loading
so a missing artifact returns `None` instead of leaking repository-specific
exceptions:
...
- `mapped_to_standard` reports `already_present` when the target standard fact
  exists with the same value;
- `mapped_to_standard` reports `conflict` and does not overwrite when the
  target standard fact exists with a different value;
...
- `_matching_canonical_facts` compares metric id, entity scope, period scope,
  ...
  value when deciding whether a match is `already_present` or `conflict`.
...
```bash
cd financial-report-analysis
uv run pytest tests/unit/test_p5_recompute.py tests/unit/test_p5_dataset.py tests/unit/test_p5_turtle_export.py -q
```
